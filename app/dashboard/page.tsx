...
export default function InventoryPage() {
  ...
  const [sharingId, setSharingId] = useState<string | null>(null) // To show loading state on share button
  ...
  const handleNativeShare = async (e: React.MouseEvent, prop: Property) => {
    ...
    try {
      ...
      // B. Detect correct extension (Important for WhatsApp)
      const mimeType = blob.type // e.g. "image/jpeg"
      const ext = mimeType.split('/')[1] || 'jpeg' 
      const file = new File([blob], `property.${ext}`, { type: mimeType })
      ...
      if (navigator.canShare && navigator.canShare(shareData)) {
        ...
        // Fallback: If device doesn't support file sharing, send link
      ...
      // E. Fallback to Link Share on error
      try {
        await navigator.share({
          title: prop.title,
          text: shareText,
